...
declare -A EXCLUDED_FILES=(
  ...
  ["crates/app/src/trust_actions.rs"]="developer-facing tracing::error! log strings only, verified in full; every failure reaches the user as a typed UserNotice built by notice_retry (HANDOFF-038)"
  ...
  ["crates/app/src/rfc059_cache_measurement.rs"]="test-only module (#[cfg(test)] in main.rs), not production code -- same exemption as runtime_isolation_tests.rs; its println! output is a one-time developer measurement pass, not user-facing"
  ["crates/app/src/rfc060_duplication_measurement.rs"]="test-only module (#[cfg(test)] in main.rs), not production code -- same exemption as rfc059_cache_measurement.rs; its println! output is a one-time developer measurement pass, not user-facing"
  ...
  ["crates/app/src/result_launch.rs"]="developer-facing tracing::warn! strings and launcher program names/arguments (xdg-open, open, explorer.exe, -R, --, /select,) only, verified in full; a refused launch reaches the user only as a typed UserNotice (HANDOFF-041, Tasks 065 and 070)"
  ...
  ["crates/app/src/search_model.rs"]="no display text: a model handle and its #[cfg(test)] module only, verified in full (Task 055)"
