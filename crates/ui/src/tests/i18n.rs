...
// Task 066's "one Japanese word for folder" property (「フォルダー」, not
// 「フォルダ」) is now one row in `crate::tests::glossary`'s `GLOSSARY`,
// checked by `default_ui_copy_follows_the_glossary` (Task 100).
