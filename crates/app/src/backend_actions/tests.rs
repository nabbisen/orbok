...
// ── Row 2: reset (Review 253 §2.4) ───────────────────────────────────────

/// (a) Under a write lock the reset fails; the reload reads the intact
/// catalog, so the list is unchanged.
#[test]
fn a_reset_that_fails_under_a_write_lock_leaves_the_list_as_the_catalog_holds_it() {
    let profile = Profile::new();
    let mut state = AppState::default();
    ...
    reset_catalog(&profile.catalog, profile.cache(), &mut state);
    drop(_lock);
    ...
    assert_eq!(
        state
            ...
            .collect::<Vec<_>>(),
        vec![id],
        "the list still shows the folder the catalog holds"
    );
    assert_eq!(bootstrap::get_sources(&profile.catalog).unwrap().len(), 1);
}

/// (b) The reload read fails too: the `sources` table is moved aside by a
/// second connection (Review Request 246 §3), so the reset fails on it and
/// so does the read. The list stays exactly as it was before the request.
#[test]
fn a_reset_whose_reload_also_fails_leaves_the_list_as_it_was() {
    let profile = Profile::new();
    let mut state = AppState::default();
    let id = profile.add_folder(&mut state);
    let mover = rusqlite::Connection::open(profile.db()).unwrap();
    mover
        .execute_batch("ALTER TABLE sources RENAME TO sources_moved_aside;")
        .unwrap();

    reset_catalog(&profile.catalog, profile.cache(), &mut state);
    let read_failed = bootstrap::get_sources(&profile.catalog).is_err();
    mover
    ...
    assert_eq!(
        state
            ...
            .collect::<Vec<_>>(),
        vec![id],
        "the list is left as it was before the request"
    );
}

/// (d) The catalog step commits and the cache purge fails: a directory sits
/// where the cache database file belongs, so the purge cannot open it. The
/// reset reports failure, and the reload shows the truth -- the catalog no
/// longer holds the folder, so the list is empty.
#[test]
fn a_reset_that_fails_after_the_catalog_step_shows_what_the_catalog_holds() {
    let profile = Profile::new();
    let mut state = AppState::default();
    profile.add_folder(&mut state);
    let cache_db = profile.data.join(orbok_db::CACHE_FILE_NAME);
    ...
    assert_eq!(state.notice, Some(UserNotice::CatalogResetFailed));
    assert!(
        bootstrap::get_sources(&profile.catalog).unwrap().is_empty(),
        "control: the catalog step committed"
    );
    assert!(
        state.sources.is_empty(),
        "the list shows what the catalog holds: no folders"
    );
}

/// (c) A reset that succeeds clears everything, with no notice.
#[test]
fn a_reset_that_succeeds_clears_the_list() {
    let profile = Profile::new();
    let mut state = AppState::default();
    profile.add_folder(&mut state);
    ...
    assert_eq!(state.notice, None);
    assert!(bootstrap::get_sources(&profile.catalog).unwrap().is_empty());
}
