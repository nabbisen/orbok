...
| 3 | ~~**Japanese results are ranked worst-first**~~ — closed by Task 034. **PDF/DOCX/HTML snippets read the wrong bytes**, and for PDF that was the smaller half: see row 3a. | Task 034 (ranking, done), **RFC-060** (snippets) |
| **3a** | **PDF text extraction returns nothing at all.** Found 2026-09-08 by RFC-058's end-to-end test on its first run; **not in the 2026-09-01 audit**. `pdf.rs:116` passes each page's *object* ID to `lopdf::extract_text`, which takes 1-based *page numbers* — so every page comes back empty and the file is reported `PossiblyScannedPdf`. Object ID equals page number only when nothing else was allocated first, which is true of hand-built fixtures and almost no real document. **One of seven advertised formats, non-functional on real files.** One-line fix; `page_num` is computed on the line above and unused. | **RFC-060 Amendment 1 §4a.1**, and it goes first — §6 renders snippets from cached extraction segments, which are empty until this lands |
