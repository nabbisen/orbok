...
**Deferred work, recorded 2026-09-02, corrected 2026-09-03, criterion 12 closed 2026-09-05 (RFC-000 granularity clause; RFC-063 §7; Review 201 §4; Task 041).** The picker, remembered-folder handling and the flow shipped. **The selected folder never scopes the query, at all** — `SearchFolderScope` exists nowhere outside `crates/ui`, and `run_search`'s signature has no scope parameter, so neither §22 criterion 7 ("and subfolders", the default) nor criterion 8 ("this folder only") reaches the query; one missing mechanism, two unmet criteria, named separately below rather than folded under 7 alone. **Criterion 12 (default UI says "folder," not "source") is now met** — Task 041 fixed the copy the Search view shared with RFC-041 (`SearchAddSource` → "Add folder", `SearchNoSourcesBody` → "Add a folder or file so orbok can search it.") behind a new exhaustive test, not the curated array that had missed it. See [closure record](../closures/045-search-in-folder-flow-and-friendly-folder-management.md). Follow-up: RFC-060 §7 (scoping) is the only criterion still open.
