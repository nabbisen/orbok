...
**Parked since v0.4.0 — moved out of `done/` 2026-09-04 (RFC-060 §8; Task
040).** It carried `Implemented (v0.4.0)` while the only implementation,
`MockReranker`, scored candidates by passage *length* — not reranking —
and had no production caller (`HybridSearchService::with_reranker`, the
one integration point, went unreachable from `bootstrap::search.rs` and
was deleted in Task 040 along with the dead call path behind it). The
`CrossEncoderReranker` trait contract is kept, in `orbok-models`, as the
documented insertion point if this is ever built. **Reopen when** orbok's
p99 search-latency gate is met with one embedding model — a second model
is a second download and a second forward pass per query, and adding that
before the first model is fast is the wrong order (currently unmet by
roughly 4x; RFC-061 §6 has not yet made model construction a once-per-
process cost either).
